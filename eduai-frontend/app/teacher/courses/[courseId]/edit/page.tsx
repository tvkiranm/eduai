...
export default function TeacherEditCoursePage() {
  ...
  async function handleSubmit(values: CourseFormValues) {
    try {
      const updated = await api.courses.update(courseId, values);
      toast.success("Course updated");
      router.replace(`/teacher/courses/${updated.id}`);
    } catch (e) {
  ...
  return (
    <div>
      ...
      <CourseForm mode="edit" initial={course.data} onSubmit={handleSubmit} />
    </div>
  );
}
